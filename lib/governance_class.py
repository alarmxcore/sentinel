...
class GovernanceClass(object):
    ...
    def vote(self, alarmxd, signal, outcome):
        return self.go.vote(alarmxd, signal, outcome)

    # pass thru to GovernanceObject#voted_on
    def voted_on(self, **kwargs):
        return self.go.voted_on(**kwargs)

    def vote_validity(self, alarmxd):
        if self.is_valid():
            printdbg("Voting valid! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(alarmxd, models.VoteSignals.valid, models.VoteOutcomes.yes)
        else:
            printdbg("Voting INVALID! %s: %d" % (self.__class__.__name__, self.id))
            self.vote(alarmxd, models.VoteSignals.valid, models.VoteOutcomes.no)

    def get_submit_command(self):
        import alarmxlib
        obj_data = alarmxlib.SHIM_serialise_for_alarmxd(self.serialise())
    ...
    def alarmxd_serialise(self):
        import alarmxlib
        return alarmxlib.SHIM_serialise_for_alarmxd(self.serialise())
