...
class GovernanceObject(BaseModel):
    ...
    @classmethod
    def sync(self, alarmxd):
        golist = alarmxd.rpc_command('gobject', 'list')
        ...
        for item in golist.values():
            try:
                (go, subobj) = self.import_gobject_from_alarmxd(alarmxd, item)
            except Exception as e:
    ...
    @classmethod
    def import_gobject_from_alarmxd(self, alarmxd, rec):
        import decimal
        import alarmxlib
        import binascii
        import gobject_json
        ...
        # exclude any invalid model data from alarmxd...
        valid_keys = subclass.serialisable_fields()
        subdikt = {k: dikt[k] for k in valid_keys if k in dikt}

        # get/create, then sync vote counts from alarmxd, with every run
        ...
        # get/create, then sync payment amounts, etc. from alarmxd - Alarmxd is the master
        try:
            newdikt = subdikt.copy()
            newdikt['object_hash'] = object_hash
            if subclass(**newdikt).is_valid() is False:
                govobj.vote_delete(alarmxd)
                return (govobj, None)

            subobj, created = subclass.get_or_create(object_hash=object_hash, defaults=subdikt)
        except Exception as e:
            # in this case, vote as delete, and log the vote in the DB
            printdbg("Got invalid object from alarmxd! %s" % e)
            govobj.vote_delete(alarmxd)
            return (govobj, None)
    ...
    def vote_delete(self, alarmxd):
        if not self.voted_on(signal=VoteSignals.delete, outcome=VoteOutcomes.yes):
            self.vote(alarmxd, VoteSignals.delete, VoteOutcomes.yes)
        return
    ...
    def vote(self, alarmxd, signal, outcome):
        import alarmxlib
        ...
        output = alarmxd.rpc_command(*vote_command)

        # extract vote output parsing to external lib
        voted = alarmxlib.did_we_vote(output)
        ...
        if voted:
            ...
            self.sync_network_vote(alarmxd, signal)

    def sync_network_vote(self, alarmxd, signal):
        printdbg('\tSyncing network vote for object %s with signal %s' % (self.object_hash, signal.name))
        vote_info = alarmxd.get_my_gobject_votes(self.object_hash)
...
class Proposal(GovernanceClass, BaseModel):
    ...
    govobj_type = ALARMXD_GOVOBJ_TYPES['proposal']
    ...
    def is_valid(self):
        ...
        try:
            ...
            # payment address is valid base58 alarmx addr, non-multisig
            if not alarmxlib.is_valid_alarmx_address(self.payment_address, config.network):
                printdbg("\tPayment address [%s] not a valid Alarmx address for network [%s], returning False" % (self.payment_address, config.network))
                return False
            ...
            # Alarmx Core restricts proposals to 512 bytes max
    ...
    def is_expired(self, superblockcycle=None):
        ...
        expiration_window_seconds = int(
            (alarmxlib.blocks_to_seconds(superblockcycle) / 2) +
...
class Superblock(BaseModel, GovernanceClass):
    ...
    govobj_type = ALARMXD_GOVOBJ_TYPES['superblock']
    ...
    def is_valid(self):
        ...
        for addr in addresses:
            if not alarmxlib.is_valid_alarmx_address(addr, config.network):
    ...
    def hash(self):
        import alarmxlib
        return alarmxlib.hashit(self.serialise())
