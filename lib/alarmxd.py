...
class AlarmxDaemon():
    def __init__(self, **kwargs):
        ...
        # memoize calls to some alarmxd methods
    ...
    @classmethod
    def from_alarmx_conf(self, alarmx_dot_conf):
        from alarmx_config import AlarmxConfig
        config_text = AlarmxConfig.slurp_config_file(alarmx_dot_conf)
        creds = AlarmxConfig.get_rpc_creds(config_text, config.network)
    ...
    def get_my_gobject_votes(self, object_hash):
        ...
        if not self.gobject_votes.get(object_hash):
            ...
            self.gobject_votes[object_hash] = alarmxlib.parse_raw_votes(raw_votes)
    ...
    def we_are_the_winner(self):
        ...
        winner = alarmxlib.elect_mn(block_hash=current_block_hash, mnlist=mn_list)
        my_vin = self.get_current_masternode_vin()
    ...
    def estimate_block_time(self, height):
        ...
        future_seconds = alarmxlib.blocks_to_seconds(diff)
    ...
    def has_sentinel_ping(self):
        ...
        return (getinfo['protocolversion'] >= config.min_alarmxd_proto_version_with_sentinel_ping)
