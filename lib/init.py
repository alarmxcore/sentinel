...
def has_alarmx_conf():
    ...
    valid_alarmx_conf = False

    # ensure alarmx_conf exists & readable
    #
    # if not, print a message stating that Alarmx Core must be installed and
    # configured, including JSONRPC access in alarmx.conf
    try:
        f = io.open(config.alarmx_conf)
        valid_alarmx_conf = True
    except IOError as e:
        print(e)

    return valid_alarmx_conf
...
def main():
    ...
    if not has_alarmx_conf():
        print("AlarmxCore must be installed and configured, including JSONRPC access in alarmx.conf")
        sys.exit(1)
