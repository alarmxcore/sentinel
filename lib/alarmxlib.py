...
def is_valid_alarmx_address(address, network='mainnet'):
    ...
    alarmx_version = 140 if network == 'testnet' else 76
    ...
    try:
        ...
        # rescue from exception, not a valid Alarmx address
        return False

    if (address_version != alarmx_version):
        return False
...
def create_superblock(proposals, event_block_height, budget_max, sb_epoch_time, maxgovobjdatasize):
    ...
    for proposal in proposals:
        ...
        data_size = len(sb_temp.alarmxd_serialise())
...
def SHIM_serialise_for_alarmxd(sentinel_hex):
    ...
    # superblock => "trigger" in alarmxd
    if govtype_string == 'superblock':
        govtype_string = 'trigger'

    # alarmxd expects an array (will be deprecated)
    obj = [(govtype_string, obj,)]

    # re-pack
    alarmxd_hex = serialise(obj)
    return alarmxd_hex
...
def did_we_vote(output):
    ...
    try:
        detail = output.get('detail').get('alarmx.conf')
