...
sentinel_cfg = AlarmxConfig.tokenize(sentinel_config_file)
sentinel_version = "1.2.0"
min_alarmxd_proto_version_with_sentinel_ping = 70207


def get_alarmx_conf():
    if sys.platform == 'win32':
        alarmx_conf = os.path.join(os.getenv('APPDATA'), "AlarmxCore/alarmx.conf")
    else:
        home = os.environ.get('HOME')

        alarmx_conf = os.path.join(home, ".alarmxcore/alarmx.conf")
        if sys.platform == 'darwin':
            alarmx_conf = os.path.join(home, "Library/Application Support/AlarmxCore/alarmx.conf")

    alarmx_conf = sentinel_cfg.get('alarmx_conf', alarmx_conf)
